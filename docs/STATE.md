...
| Active task | P3-T7 — encode the excellence-by-default quality bar and close the cold-start contract gap. |
| Exact progress | P3-T6 merged as PR #14 (`f455f9a`): progress-first fast path, one-pass Step Pilot, compact reporting, and restricted-workflow staging are on `main`. P3-T7 adds the silent professional quality bar to SMART (invariant 9 + "Excellence by default" section + anti-patterns), agent rule 15 in CLAUDE.md, a cold-start contract test proving execution capabilities defer until Vision Lock, three new contract tests for the quality bar, and offline scenario `novice-gets-expert-quality` (9 scenarios). Versions: SMART/marketplace `2.4.0`. |
| Last evidence | Current branch: full deterministic test suite GREEN including new cold-start and quality-bar contracts; 9 offline scenarios valid; all JSON valid; Bash syntax GREEN. Local ShellCheck remains delegated to required CI. |
| Blocker / waiting on | None for implementation. Required PR checks must confirm ShellCheck and repository validation. |
| Vision Lock | CONFIRMED by repository owner on 2026-07-11 (reaffirmed and extended): SMART must be the complete project control brain, deliver expert-grade project quality to novices by default, and remain lean enough that orchestration never stalls or materially slows project progress. |
...
| Branch / head | `genspark_ai_developer`; based on merged PR #14 (`f455f9a`) with P3-T7 in progress. |
...
| CORE-006 | Output quality for novices depended on the user knowing what to ask for. | SMART contract before P3-T7: no explicit quality bar; a novice's "do whatever is simplest" could yield novice-grade output. | The core product promise (expert-grade result from one activation) was not contractually guaranteed. | Merge the excellence-by-default contract, anti-patterns, contract tests, and offline scenario. | IN PROGRESS |
...
|---|---|---|---|---|
| 2026-07-11 / current branch | Added the excellence-by-default quality contract (invariant, section, anti-patterns, agent rule), cold-start deferral test, quality-bar contract tests, and offline scenario `novice-gets-expert-quality`; released `2.4.0`. | Guarantee that one SMART activation gives a novice expert-team output quality without adding orchestration weight. | Full test suite GREEN including new contracts; 9 scenarios valid; JSON and Bash checks GREEN. | SMART, agent contract, tests, scenarios, README, manifests, changelog, this STATE. |
...
## Runway
1. **NEXT — complete and merge P3-T7 excellence-by-default:** Squash to one commit, sync with `origin/main`, push, and require all checks GREEN. Completion evidence: quality-bar invariant/section/anti-patterns in SMART, agent rule 15, cold-start deferral test, quality contract tests, scenario 9 valid, version `2.4.0`, clean tree.
2. **THEN — verify the quality bar in a real cold-start run:** Exercise SMART on a fresh toy project and confirm the novice path yields expert defaults without quality questions or ceremony. Completion evidence: a short documented trace or a focused contract correction; no new framework.
