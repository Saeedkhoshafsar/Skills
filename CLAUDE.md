...
15d. **Harness-compat before thrash.** Model↔Claude Code friction (illegal content blocks, tool-loop shape, slash namespace, plugin path protocol — not API credit alone) → look up SMART `references/HARNESS-COMPAT.md` first, apply SOLVED recipes, register OPEN when new, promote when solved. Ensure always-on pointer via `scripts/ensure-user-claude-md.sh` → `~/.claude/CLAUDE.md` so non-SMART sessions still escalate to `/smart:smart`. Never teach bare `/smart` as a plugin path. **Consumer update path (document + teach):** `claude plugin marketplace update saeed-skills` → `claude plugin update smart@saeed-skills` → `bash …/ensure-user-claude-md.sh` → **restart** → `/smart:smart` (all four steps; not only the first two).
